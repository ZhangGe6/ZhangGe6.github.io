...
# 动机
同样是因为接手学长的项目。由于学长的项目是用C/C++写的，提供了部分文件的编译命令行，但不足以成功编译，需要我在做一些调整和增补。由于之前对g++/gcc编译过程和编译选项只有浅显了解，所以正好学习记录一下。
# g++和gcc
...
g++ -c hello.s -o hello.o  对汇编文件进行编译，生成了目标文件
g++ hello.o -o hello 对目标文件进行链接，生成可执行文件
...
程序test.c是在linux上使用c连接mysql，这个时候我们需要去mysql官网下载MySQL Connectors的C库，下载下来解压之后，有一个include文件夹，里面包含头文件，还有一个lib文件夹，里面包含二进制so文件。
