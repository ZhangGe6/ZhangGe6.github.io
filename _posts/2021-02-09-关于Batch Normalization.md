...
> 本文主体内容为对Medium文章[Batch normalization in 3 levels of understanding](https://towardsdatascience.com/batch-normalization-in-3-levels-of-understanding-14c2da90a338#b93c)的总结梳理，有需要的读者可以阅读原文，写得很好。

`Batch-Normalization (BN)` 通过使用当前batch数据的均值mean和方差variance对它们进行标准化处理，可以使得模型的训练更加**快速**和**稳定**。实践中会把BN作为一个层，一般放在非线性激活函数的上一层或者下一层。一个示意的效果如[图1, 图2](https://towardsdatascience.com/batch-normalization-in-3-levels-of-understanding-14c2da90a338#b93c)所示。

![](/assets/img/20210209/unBN.jpeg)

<center>图1   一个未使用BN的Multilayer Perceptron (MLP)(注意hidden layers的边权值)</center>

![](/assets/img/20210209/BN.jpeg)

<center>图2   一个使用了BN的Multilayer Perceptron (MLP)</center>
...
> 我的理解是，对于一张特征图，某一个pixel的normalization是基于它所在channel上HxW个tensor计算的；那**batch** normalization只是对应把计算的范围对应扩展到batch_sizexHxW而已。

注意，一个channel对应一对均值和方差。一个channel对应一对$$\gamma$$ 和 $$\beta$$ 。这也是在Pytorch中定义[BatchNorm2d](https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm2d.html)层时，需要传入的是channel_num的缘故。[参考](https://blog.csdn.net/qq_27261889/article/details/87284076)和[参考](https://blog.csdn.net/weixin_38314865/article/details/104327852)
