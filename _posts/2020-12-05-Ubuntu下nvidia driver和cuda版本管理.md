...
表格内容可能过时，可以点击[nvidia官网链接](https://docs.nvidia.com/cuda/cuda-toolkit-release-notes/index.html)查看最新更新。
## cuda与pytorch
...
lsmod | grep nouveau
```
> 接下来的操作，如果当前是在本机图形化界面进行的，则需首先`Ctrl+alt+F1`进入字符界面(操作完成后`Ctrl+alt+F7`返回图形化界面)，如果是远程服务器方式连接，则直接执行下面的操作即可。

然后关闭图形化界面
```bash
sudo service lightdm stop
```
> 如果提示`unit lightdm.service not loaded`，则先安装LightDm：`sudo apt install lightdm`，安装完毕后跳出一个界面，选择`lightdm`，再执行上述命令。
...
source ~/.bashrc
```
叮叮，安装完成。但如果要切换新安装的版本，需要下面的步骤切换一下（假设安装过中没有选择覆盖已有`cuda`版本）。
## 切换cuda版本
