...
- 修改镜像源([参考](https://lilianweng.github.io/lil-log/2019/11/10/self-supervised-learning.html))
...
- **conda瘦身**
  ...
  ```bash
  conda clean -p    # 删除没有的包
  conda clean -t    # 删除保存的tar包
  ```

  [参考](https://blog.csdn.net/marsjhao/article/details/62884246)

  使用Ubuntu的话，Anaconda默认安装在主目录下，但当Anaconda文件夹太大影响到机器使用时，这个命令可以解燃眉之急。当然最好是把Anaconda安装在其他有较大空间的位置。