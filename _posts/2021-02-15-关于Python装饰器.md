...
> 本文的内容主要转载自[runoob-Python 函数装饰器](https://www.runoob.com/w3cnote/python-func-decorators.html)...的评论区笔记。高手在民间，这个笔记写得确实非常好，简直要压过教程本身的光芒。
...
谈装饰器前，还要先要明白一件事，Python 中的函数和 Java、C++不太一样，Python中的函数可以像普通变量一样当做参数传递给另外一个函数，例如：
...
> 笔者注：如果在上上面那个不加装饰器情况下，直接进行`foo = use_logging(foo) `可不可以达到同样的效果呢？答案是不，执行这样一句实际上是执行了`use_logging(foo) `函数然后返回这个函数的返回值。可以发现，`foo()`直接被执行了，所谓返回的`foo`是`None`。
...
> 这里的参数是如何传递进去的呢？仍用函数可作为对象传递（未加括号时，加上括号就是调用这个函数了）的视角来看，则是十分容易：进入修饰的foo是一个光秃秃的函数，不带有参数，修饰完毕后，foo被修饰成了wrapper，这时候再加上后面的参数(\*args, \*\*kwargs)，即wrapper(\*args, \*\*kwargs)，结束。
